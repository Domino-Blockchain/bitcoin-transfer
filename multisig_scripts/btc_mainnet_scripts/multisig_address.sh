...
# export XPUB_01=$(bdk-cli --network bitcoin key derive --xprv $XPRV_01 --path "m/84'/1'/0'/0" | jq -r ".xpub")
...
export MULTI_DESCRIPTOR_00=$(bdk-cli --network bitcoin compile "thresh(2,pk($DESCRIPTOR_00),pk($XPUB_01),pk($XPUB_02))" | jq -r '.descriptor')
export MULTI_DESCRIPTOR_01=$(bdk-cli --network bitcoin compile "thresh(2,pk($XPUB_00),pk($XPUB_01),pk($XPUB_02))" | jq -r '.descriptor')
export MULTI_DESCRIPTOR_02=$(bdk-cli --network bitcoin compile "thresh(2,pk($XPUB_00),pk($XPUB_01),pk($DESCRIPTOR_02))" | jq -r '.descriptor')
...
export MULTI_ADDRESS=$(bdk-cli --network bitcoin wallet --wallet wallet_name_msd00 --descriptor $MULTI_DESCRIPTOR_00 get_new_address)
export MULTI_ADDRESS_COPY_2=$(bdk-cli --network bitcoin wallet --wallet wallet_name_msd01 --descriptor $MULTI_DESCRIPTOR_01 get_new_address)
export MULTI_ADDRESS_COPY_3=$(bdk-cli --network bitcoin wallet --wallet wallet_name_msd02 --descriptor $MULTI_DESCRIPTOR_02 get_new_address)
