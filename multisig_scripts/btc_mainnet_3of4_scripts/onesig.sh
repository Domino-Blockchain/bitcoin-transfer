...
# to send TX
export TO_ADDRESS="bc1qaf5v2m7dhkl0rpxswzn6ptqy0wy03jh3fnnj8l" # Electrum Wallet address
export AMOUNT="550" # More than dust limit
echo "Sending $AMOUNT sat to $TO_ADDRESS"
export JSON_OUTPUT=$(bdk-cli --network bitcoin wallet --wallet wallet_name_msd00 --descriptor $MULTI_DESCRIPTOR_00 create_tx --to $TO_ADDRESS:$AMOUNT --external_policy "{\"$CHANGE_ID\": [0,1,3]}")
